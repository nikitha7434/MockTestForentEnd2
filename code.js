...
let student2 =JSON.parse(student1);
let searchtext = document.getElementById("search_input");
let searchbtn = document.getElementById("search-btn");
let incrementoder = document.getElementById("sort-inr");
let sortmarks =document.getElementById("sort-marks");
let passbtn = document.getElementById("sort-pass");
let decrementoder = document.getElementById("sort-dcr");
let classbtn = document.getElementById("sort-class");
let genderbtn = document.getElementById("sort-gender");

let tabledata = document.getElementById("table_t");
let tabledata2 = document.getElementById("table_t2"); 


addDataToHTML(student2);
searchbtn.addEventListener('click',searchDataByInpt);
incrementoder.addEventListener('click',sortDataIncremnetOrder);
decrementoder.addEventListener('click',sortDataDecremnetOrder);
sortmarks.addEventListener('click',sortDataByMarksAsscedingOder);
passbtn.addEventListener('click',sortDataByPassing);
classbtn.addEventListener('click',sortedDataByClass);
genderbtn.addEventListener('click',sortDataByGender);

function addDataToHTML(studet){
    tabledata2.innerHTML="";
    tabledata.innerHTML="";
    tabledata.innerHTML = ' <th>ID</th> <th>Name </th><th>Gender</th><th>Class</th> <th>Marks</th><th>Passing</th><th>Email</th>';

    studet.map((item) => {
   
        let createrow = document.createElement('tr');
     
     createrow.innerHTML = '<td>'+ item.id +'</td>'+
        ...
        tabledata.append(createrow);
    });

}

function sortDataIncremnetOrder(){

  let sortedArrayIncr  = student2.sort((a,b) => {
        if(a.first_name < b.first_name) return-1;
        else if(a.first_name>b.first_name) return 1;
        else return 0;
    });
   addDataToHTML(sortedArrayIncr);
}

function sortDataDecremnetOrder(){

    let sortedArraydec  = student2.sort((a,b) => {
        if(a.first_name > b.first_name) return-1;
        else if(a.first_name<b.first_name) return 1;
        else return 0;
    });
   addDataToHTML(sortedArraydec);

}

function sortDataByMarksAsscedingOder(){

    let sortMAerksAsc = student2.sort((a,b) => {
    ...
    addDataToHTML(sortMAerksAsc);

}

function sortDataByPassing(){

    let passedstudent =  student2.filter((item) => {
        return item.passing;
    });
    addDataToHTML(passedstudent);

}

function sortedDataByClass(){

    let sotbyclass  = student2.sort((a,b) => {
    ...
    addDataToHTML(sotbyclass);
    
}

function searchDataByInpt(){
    
    console.log("hello");

    let userchoice = searchtext.value.toLowerCase();

     let studentdetl = student2.filter((item) => {
   
        if((item.first_name.toLowerCase().includes(userchoice)) || 
            (item.last_name.toLowerCase().includes(userchoice)) || (item.email.toLowerCase().includes(userchoice))){
            return item;
        }
        

     });
     addDataToHTML(studentdetl);
...
function sortDataByGender(){
    let sortedgenFemale =student2.filter((item) => {

       return item.gender.toLowerCase() == 'female';
    });
    addDataToHTML(sortedgenFemale);
     sortedDataByMale();
    
}

function sortedDataByMale(){
    let sortedgenMale = student2.filter((item) => {

        return item.gender.toLowerCase() == 'male' ;

    });
    addDataTOHTMLTAble2(sortedgenMale);
}



function addDataTOHTMLTAble2(studet){
    

    tabledata2.innerHTML="";
    tabledata2.innerHTML = ' <th>ID</th> <th>Name </th><th>Gender</th><th>Class</th> <th>Marks</th><th>Passing</th><th>Email</th>';
